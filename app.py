    ...
    if OPENAI_API_KEY:
        ...
        def ask_question(i=0):
            user_question = st.text_input(
                "Ask a question about your Legal Document?", key=i)
            if user_question:
                docs = knowledge_base.similarity_search(user_question)
                # st.write(docs)

                llm = "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"
            ...
            if pswd:
                with st.spinner("Encrypting..."):
                    ...
                    st.download_button(
                        ...
                        data=open(
                            f"{pdf.name.split('.')[0]}_encrypted.pdf", "rb").read(),
            ...
            for page in pdf_reader.pages:
                if "/Annots" in page:
                    for annot in page["/Annots"]:
                        ...
                        st.write("***********")
                        annotation = {
                            "subtype": obj["/Subtype"], "location": obj["/Rect"]}
                        st.write(annotation)
        elif option == "ChatPDF💬":
            ChatPDF(text)
